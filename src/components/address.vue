        ...
        <div class="container">
            <div class="checkout-addr">
                ...
                <div class="addr-list-wrap">
                    <div class="addr-list">
                        <ul>
                            <li v-for="(item, index) in filterAddressList" v-bind:class="{'check': index==currentIndex}" v-on:click="currentIndex=index">
                                <dl>
                                    <dt>{{item.userName}}</dt>
                                ...
                                <div class="addr-opration addr-edit">
                                    <a href="javascript:;" class="addr-edit-btn" v-on:click="checkAddress">
                                ...
                                <div class="addr-opration addr-del">
                                    <a href="javascript:;" class="addr-del-btn" v-on:click="delconfig(index)">
                                ...
                                <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                                    <a href="javascript:;" class="addr-set-default-btn" v-on:click="setDefault(item.addressId)"><i>设为默认</i></a>
                                </div>
                                <div class="addr-opration addr-default" v-if="item.isDefault">默认地址</div>
                            </li>
                            <li class="addr-new">
                                <div class="add-new-inner">
                                    <i class="icon-add"><svg class="icon icon-add"><use xlink:href="#icon-add"></use></svg></i>
                                    <p>添加新地址</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div class="shipping-addr-more" v-on:click="loadMore(addressList.length)">
                ...
                <div class="shipping-method-wrap">
                    <div class="shipping-method">
                        <ul>
                            <li v-bind:class="{'check': shoppingMehotd==1}" v-on:click="shoppingMehotd=1">
                                <div class="name">标准配送</div>
                                <div class="price">Free</div>
                            </li>
                            <li v-bind:class="{'check': shoppingMehotd==2}" v-on:click="shoppingMehotd=2">
                ...
                <div class="md-modal modal-msg md-modal-transition" id="showModal" v-bind:class="{'md-show':delFlag}">
                    <div class="md-modal-inner">
                        <div class="md-top">
                            <button class="md-close" v-on:click="delFlag=false; showOverLayIsShow=false;">关闭</button>
                        </div>
                        <div class="md-content">
                            ...
                            <div class="btn-wrap col-2">
                                ...
                                <button class="btn btn--m btn--red" id="btnModalCancel" v-on:click="delFlag=false; showOverLayIsShow=false;">No</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="md-modal modal-msg md-modal-transition" id="showModalw" v-bind:class="{'md-show':changeInfo}">
                    <div class="md-modal-inner">
                        <div class="md-top">
                            <button class="md-close" v-on:click="changeInfo=false; showOverLayIsShow=false;">关闭</button>
                        </div>
                        <div class="md-content">
                            <div class="confirm-tips">
                                <div class="md-form-item">
                                    ...
                                    <div class="md-form-item__content" style="margin-left: 80px;">
                                        <div class="el-input">
                                            <input type="text" autocomplete="off" class="md-input__inner" v-model="userName">
                                        </div>
                                    </div>
                                </div>
                                <div class="md-form-item">
                                    ...
                                    <div class="md-form-item__content" style="margin-left: 80px;">
                                        <div class="el-input">
                                            <input type="text" autocomplete="off" class="md-input__inner" v-model="streetName">
                                        </div>
                                    </div>
                                </div>
                                <div class="md-form-item">
                                    ...
                                    <div class="md-form-item__content" style="margin-left: 80px;">
                                        <div class="el-input">
                                            <input type="text" autocomplete="off" class="md-input__inner" v-model="tel">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="btn-wrap col-2">
                                <button class="btn btn--s" id="btnModalConfirms" v-on:click="saveAddress">保存</button>
                                <button class="btn btn--s btn--red" id="btnModalCancels" v-on:click="changeInfo=false; showOverLayIsShow=false;">取消</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="md-overlay" id="showOverLay" v-if="showOverLayIsShow"></div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        filterAddressList() {
            return this.addressList.slice(0, this.initNum);
        }
    },
    methods: {
        addressView() {
            this.$http.get('/static/data/address.json')
                .then(response => {
        ...
        delconfig(index) {
            ...
            this.delFlag = true;
            this.showOverLayIsShow = true;
        },
        delAddress() {
            this.addressList.splice(this.delCurrent, 1);
            this.delFlag = false;
            this.showOverLayIsShow = false;
        },
        checkAddress() {
