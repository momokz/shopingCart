                            ...
                            <li v-for="(item, index) in productList">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascript:void 0" class="item-check-btn" v-bind:class="{'check': item.checked}" v-on:click="selectedItem(item)">
                                ...
                                <div class="cart-tab-2">
                                    <div class="item-price">{{item.productPrice | filterMoney('元')}}</div>
                                </div>
                                ...
                                <div class="cart-tab-4">
                                    <div class="item-price-total">{{item.productQuantity*item.productPrice | filterMoney('元')}}</div>
                                </div>
                                <div class="cart-tab-5">
                                    <div class="cart-item-operation">
                                        <a href="javascript:void 0" class="item-edit-btn" v-on:click="delconfig(index)">
                ...
                <div class="cart-foot-wrap">
                    <div class="cart-foot-l">
                        <div class="item-all-check">
                            <a href="javascript:void 0" v-on:click="selectedAllItem(true)">
                                <span class="item-check-btn" v-bind:class="{'check': selectedAll}"><svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg></span>
                                <span>全选</span>
                            </a>
                        </div>
                        <div class="item-all-del">
                            <a href="javascript:void 0" class="item-del-btn" v-on:click="selectedAllItem(false)">
                                <span>取消全选</span>
                            </a>
                        </div>
                    </div>
                    <div class="cart-foot-r">
                        <div class="item-total">
                            Item total: <span class="total-price">{{totalMoney | filterMoney('元')}}</span>
                        </div>
...
export default {
    ...
    methods: {
        shoppingCartView() {
            this.$http.get('/static/data/cartData.json')
                .then(response => {
        ...
        selectedAllItem(flag) {
            this.selectedAll = flag;
            this.productList.forEach((item, index) => {
                this.$set(item, 'checked', this.selectedAll);
            })
        },
        calceTotalMoney() {
            this.totalMoney = 0;
        ...
        delproduct() {
            this.productList.splice(this.delCurrent, 1);
            this.delFlag = false;
        }
    },
    filters: {
        filterMoney(value, type) {
